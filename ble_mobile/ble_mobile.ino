...
static char local_ble_name[BT_NAME_LEN] = "SPR-CENTRAL";
static struct ble_state_s *s_ble_state = NULL;

static int g_ble_bonded_device_num;
static struct ble_cccd_s **g_cccd = NULL;

static uint16_t g_last_cccd_handle = BLE_GATT_INVALID_ATTRIBUTE_HANDLE;

static bool is_connected_to_device1 = false;
static bool is_connected_to_device2 = false;

const char* filePath = "/mnt/sd0/Honden-English.mp3";
                                        ...
                                        bool connected, uint8_t reason)
{
  ...
  /* デバイスが切断された場合、スキャンを再度開始する */
  if (!connected)
    {
      int ret = ble_start_scan(false);
      if (ret != BT_SUCCESS)
        {
          printf("%s [BLE] 再スキャンの開始に失敗しました。ret = %d\n", __func__, ret);
        }
      else
        {
          printf("%s [BLE] デバイス切断後に再スキャンを開始します。\n", __func__);
        }
    }
}
...
static void on_scan_result(BT_ADDR addr, uint8_t *data, uint8_t len)
{
  const char target_device_name1[] = "SPR-PERIPHERAL";
  const char target_device_name2[] = "SPR-PERIPHERAL2";
  ...
  if (strcmp((char *)eir.data, target_device_name1) != 0 &&
      strcmp((char *)eir.data, target_device_name2) != 0)
    {
      printf("%s [BLE_GATT] BLE target_device_name %s ignored.\n", __func__, eir.data);
      goto bye;
    }
  else if (strcmp((char *)eir.data, target_device_name1)==0)
  {
   is_connected_to_device1 = true; 
  }
  else if (strcmp((char *)eir.data, target_device_name2)==0)
  {
   is_connected_to_device2 = true; 
  }
...
static void encryption_result(uint16_t handle, bool result)
{
  uint16_t buf = 1; /* request notification enable */

  printf("Encryption result(connection handle = %d) : %s\n",
         handle, (result) ? "Success" : "Fail");

  ble_descriptor_write(handle, g_last_cccd_handle, (uint8_t *)&buf, 2);
}
...
static void on_notify(struct ble_gatt_char_s *ble_gatt_char)
{
  ...
  printf("%s [BLE] \n", __func__);
  Serial.begin(115200);
  if (is_connected_to_device1==true)
  {
    printf("conect1\n");
    player.mp3play(filePath); // ファイル再生
    is_connected_to_device1=false;
  }
  if (is_connected_to_device2==true)
  {
    printf("conect2\n");
    player.mp3play(filePath); // ファイル再生
    is_connected_to_device2=false;
  }
      ...
      for (j = 0; j < srv->char_count; j++, ch++)
        {
          ...
          if (ch->cccd_handle != BLE_GATT_INVALID_ATTRIBUTE_HANDLE)
            {
              g_last_cccd_handle = ch->cccd_handle;
            }
        }
...
void setup()
{
  ...
  player.begin(); // 初期化
  player.volume(1); // ボリューム設定
  player.setFs(16000); // サンプリングレート設定
